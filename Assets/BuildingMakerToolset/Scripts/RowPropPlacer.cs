...
namespace BuildingMakerToolset.PropPlacer
{
    ...
    public class RowPropPlacer : BuildingUnit
    {
        ...
        public void TransferGroupMask(PropGroup[] fromGroups, bool[] fromMask)
        {
            UpdateGroupMask();
        ...
        public void SetPrevewMeshIndex(int index)
        {
        ...
        public List<RowPropPlacer> GetNeighborsWithScript<T>() where T : MonoBehaviour
        {
            ...
            List<RowPropPlacer> neighborObjects = SortedNeighbors( FindObjectsOfType<T>().Where(x=>x.GetComponent<RowPropPlacer>()!=null).Select(x=>x.GetComponent<RowPropPlacer>()).ToList(),this);
        ...
        public static List<RowPropPlacer> SortedNeighbors(List<RowPropPlacer> unsorted, RowPropPlacer startFrom = null)
        {
            List<RowPropPlacer> neighborObjects = new List<RowPropPlacer>();
            if (startFrom == null || !unsorted.Contains( startFrom ))
                startFrom = unsorted[0];
            neighborObjects.Add( startFrom );
            RowPropPlacer startSpawner = startFrom;
            RowPropPlacer endSpawner = startFrom;

            bool foundStart = false;
            bool notFound = false;
            while (!foundStart && !notFound)
            {
                notFound = true;
                for (int i = 0; i < unsorted.Count; i++)
                {
                    RowPropPlacer curCheckSpawner = null;
                    if (unsorted[i].TryGetComponent( out RowPropPlacer spnr ))
                    {
                        curCheckSpawner = spnr;
                    }
                    else
                        continue;
                    if (Vector3.Distance( curCheckSpawner.OriginPivotWorldPosition, endSpawner.SpawnPivotWorldPosition ) < 0.01f)
                    {
                        if (curCheckSpawner == startSpawner)
                            foundStart = true;
                        endSpawner = curCheckSpawner;
                        neighborObjects.Add( unsorted[i] );
                        unsorted.RemoveAt( i );
                        notFound = false;
                        break;
                    }
                    else if (Vector3.Distance( startSpawner.OriginPivotWorldPosition, curCheckSpawner.SpawnPivotWorldPosition ) < 0.01f)
                    {
                        if (curCheckSpawner == endSpawner)
                            foundStart = true;
                        startSpawner = curCheckSpawner;
                        neighborObjects.Insert( 0, unsorted[i] );
                        unsorted.RemoveAt( i );
                        notFound = false;
                        break;
                    }
